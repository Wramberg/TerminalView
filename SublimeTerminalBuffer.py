...
class SublimeTerminalBuffer():
    def __init__(self, sublime_view, title, logger):
        ...
        hist = settings.get("terminal_view_scrollback", 5000)
        self._view.terminal_view_emulator = terminal_emulator.TerminalEmulator(80, 24, history=hist)
        self._view.terminal_view_size = (80, 24)
    ...
    def update_terminal_size(self, nb_rows, nb_cols):
        self._view.terminal_view_size = (nb_rows, nb_cols)
...
class TerminalViewUpdate(sublime_plugin.TextCommand):
    def run(self, edit):
        ...
        if len(dirty_lines) > 0:
            ...
            self._update_lines(edit, dirty_lines)
            self._scroll_st_viewport()

            update_time = time.time() - start
            log = "Updated st3 view in %.3f ms" % (update_time * 1000.)
            self.view.terminal_view_logger.log(log)

        self.view.terminal_view_emulator.clear_dirty_lines()
    ...
    def _scroll_st_viewport(self):
        # Get the top line that should be visible. If we use the bottom one it
        # scrolls a bit too far
        display_height = self.view.terminal_view_size[0]
        total_nb_lines = len(self.view.terminal_view_buffer_contents)
        top_visible_line = total_nb_lines - display_height

        print(total_nb_lines)
        print(self.view.terminal_view_emulator.cursor())

        # Compute desired position and set it
        desired_pos = top_visible_line * self.view.line_height()
        self.view.set_viewport_position([0, desired_pos], animate=False)

    def _update_cursor(self):
        cursor_y, cursor_x = self.view.terminal_view_emulator.cursor()
        tp = self.view.text_point(cursor_y, cursor_x) # TODO OPTMIMIZE THIS!!!
        self.view.sel().clear()
        self.view.sel().add(sublime.Region(tp, tp))

    def _update_lines(self, edit, dirty_lines):
        self.view.set_read_only(False)
        for line_no in dirty_lines.keys():
            # We may get line numbers outside the current display if it was
            # resized to be smaller
            # if line_no > len(display) - 1:
            # if not self.view.terminal_view_emulator.line_inside_display(line_no):
                # break
            ...
            # Update the line
            # offset = self.view.terminal_view_emulator.offset()
            # content = self.view.terminal_view_emulator.get_display_line_content(line_no)
